...
function renderMarkdownBlock(value) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (pair) {
      ...
      blocks.push(`<div class="md-pairs">${pairs.map((item) => `<div class="md-pair"><span class="md-pair-label">${inlineMarkdown(item.label)}</span><span class="md-pair-sep">:</span><span class="md-pair-value">${inlineMarkdown(item.value)}</span></div>`).join("")}</div>`);
      continue;
    }
