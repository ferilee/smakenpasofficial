...
const statements = [
  ...
  `CREATE TABLE IF NOT EXISTS galleries (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, slug TEXT NOT NULL UNIQUE, category TEXT NOT NULL DEFAULT 'Kegiatan', description TEXT NOT NULL DEFAULT '', cover_url TEXT NOT NULL DEFAULT '', album_url TEXT NOT NULL DEFAULT '', show_on_home INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`,
  ...
  `CREATE TABLE IF NOT EXISTS student_infos (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, category TEXT NOT NULL DEFAULT 'Info Siswa', content TEXT NOT NULL DEFAULT '', is_priority INTEGER NOT NULL DEFAULT 0, attachment_url TEXT NOT NULL DEFAULT '', status TEXT NOT NULL DEFAULT 'active', published_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`,
  `CREATE TABLE IF NOT EXISTS student_services (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, description TEXT NOT NULL DEFAULT '', url TEXT NOT NULL DEFAULT '#', icon TEXT NOT NULL DEFAULT 'book', sort_order INTEGER NOT NULL DEFAULT 0, is_active INTEGER NOT NULL DEFAULT 1)`,
  `CREATE TABLE IF NOT EXISTS student_announcements (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, content TEXT NOT NULL DEFAULT '', is_priority INTEGER NOT NULL DEFAULT 0, attachment_url TEXT NOT NULL DEFAULT '', status TEXT NOT NULL DEFAULT 'active', published_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`,
  `CREATE TABLE IF NOT EXISTS student_agendas (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, start_date TEXT NOT NULL, end_date TEXT NOT NULL DEFAULT '', location TEXT NOT NULL DEFAULT '', description TEXT NOT NULL DEFAULT '', registration_url TEXT NOT NULL DEFAULT '', status TEXT NOT NULL DEFAULT 'scheduled', created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`,
...
export function migrate() {
  const transaction = sqlite.transaction(() => {
    ...
    if (!profileColumns.some((column) => column.name === "former_principals")) {
      sqlite.run("ALTER TABLE school_profile ADD COLUMN former_principals TEXT NOT NULL DEFAULT '[]'");
    }
    ...
    if (!settingsColumns.some((column) => column.name === "editor_permissions")) {
      sqlite.run("ALTER TABLE school_settings ADD COLUMN editor_permissions TEXT NOT NULL DEFAULT '[]'");
    }
    ...
    for (const field of ["instagram", "tiktok", "facebook", "youtube"]) {
      if (!majorColumns.some((column) => column.name === field)) {
        sqlite.run(`ALTER TABLE majors ADD COLUMN ${field} TEXT NOT NULL DEFAULT ''`);
      }
    }

    const galleryColumns = sqlite.prepare("PRAGMA table_info(galleries)").all() as { name: string }[];
    if (!galleryColumns.some((column) => column.name === "album_url")) {
      sqlite.run("ALTER TABLE galleries ADD COLUMN album_url TEXT NOT NULL DEFAULT ''");
    }
